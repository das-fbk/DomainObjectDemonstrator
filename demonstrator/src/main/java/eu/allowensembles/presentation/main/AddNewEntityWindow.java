...
//	private MapViewerComponentBuilder mvcb = new MapViewerComponentBuilder();
	...
	private URL resource = getClass().getResource("/images/waypoint_white.png");

	private JCheckBox bicycleCheckBox;
	...
	public AddNewEntityWindow(MainWindow mainWindow) {
		...
		JLabel lblEntityRouteDetails = new JLabel("Entity route details");
		lblEntityRouteDetails.setBounds(24, 106, 158, 16);
		entityInformationPanel.add(lblEntityRouteDetails);

	//	mapPanel = mvcb.buildViewer();
	//	mapPanel.setBounds(24, 134, 466, 309);
	//	mapPanel.setLayout(null);
	//	entityInformationPanel.add(mapPanel);
	//	mapPanel.setOverlayPainter(new CompoundPainter<JXMapViewer>());
	...
	//	mapPanel.repaint();
		...
		bicycleCheckBox = new JCheckBox("Bicycle");
		bicycleCheckBox.setBounds(10, 146, 97, 23);
		entityPreferencesPanel.add(bicycleCheckBox);
		...
		// update map when is visibile
		/*
		mapPanel.addAncestorListener(new AncestorListener() {
			@Override
			public void ancestorAdded(AncestorEvent event) {
				if (isShowing()) {
					UserData ud = controller.getUserData(nameText.getText());
					if (ud.getSelectedAlternative() != null) {
						mvcb.addRoute(mapPanel, ud.getSelectedAlternative()
								.getLegs());
					}
					// add starting and ending point
					List<GeoPosition> points = Arrays.asList(ud.getMapIcon()
							.getPosition(), ResourceLoader.getMapInfo()
							.getDestination());
					mapPanel.setOverlayPainter(new CompoundPainter<JXMapViewer>());
					mvcb.displayPoints(mapPanel, points, Color.RED, resource);
				}
	...
	public boolean getTtBicycle() {
		return bicycleCheckBox.isSelected();
	}
	...
	/*
	 * public void setPreferences(Preferences preferences) { // transportation
	 * type carCheckBox.setSelected(preferences.isTtCar());
	 * flexibusCheckbox.setSelected(preferences.isTtFlexibus());
	 * walkCheckbox.setSelected(preferences.isTtWalk());
	 * carpoolingCheckBox.setSelected(preferences.isTtCarpooling()); // payment
	 * method cashCheckBox.setSelected(preferences.isPmCash());
	 ...
	 * 
	 * // user weights sliderTime.setValue((int) (100 *
	 * preferences.getTTweight())); sliderCost.setValue((int) (100 *
	 * preferences.getCweight())); sliderReliability.setValue((int) (100 *
	 * preferences.getRCweight())); sliderWalkingDistance.setValue((int) (100 *
	 * preferences.getWDweight())); sliderNumOfChanges.setValue((int) (100 *
	 * preferences.getNCweight()));
	 * 
	 * }
	 */
	...
	/*
	 * public void save(String userName, Preferences userPreference) { if
	 * (userPreference == null) { return; } DoiBean db =
	 * controller.getCurrentDoiBean(); if (db != null && db.getName() != null &&
	 * db.getName().equals(userName)) { // existing user UserData ud =
	 * controller.getUserData(db.getName()); if (ud != null) {
	 * ud.setPreferences(userPreference); } return; } else { // new user
	 * controller.buildUserDoiBean(userName, userPreference); return; } }
	 */
	...
	/*
	 * public void reset() { nameCounter++; nameText.setText("User_" +
	 * nameCounter); nameText.setEditable(false); setPreferences(new
	 * Preferences()); }
	 */
