...
import static eu.allowensembles.presentation.main.map.MapInfo.DESTINATION_POINT;
...
public final class MapInfoLoader {
	...
	public final MapInfo loadInfo(String fileName) throws IOException {
		...
		mapInfo.setParkingPoints(parseStringPoints(p.getProperty(
				PARKING_POINTS, "")));
		mapInfo.setDestination(parseGeoPosition(p.getProperty(
				DESTINATION_POINT, "")));
		logger.debug("loadInfo end, loaded " + mapInfo.toString());
		return mapInfo;
	}

	private GeoPosition parseGeoPosition(String value) {
		String[] v = value.split(",");
		return new GeoPosition(Double.valueOf(v[0]), Double.valueOf(v[1]));
	}
	...
	private List<GeoPosition> parseStringPoints(String value) {
		...
		for (int i = 0; i < points.length; i++) {
			result.add(parseGeoPosition(points[i]));
		}
		return result;
	}
}
