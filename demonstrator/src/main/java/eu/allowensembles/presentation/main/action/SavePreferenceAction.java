...
public class SavePreferenceAction implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		 ...
		 * userPreference.setTtBicycle(window.getTtBicycle());
		 * 
		 ...
		 * 
		 * // do actual save. Note: is not possible right now to change user
		 * name window.save(window.getUserName(), userPreference);
		 * 
		 * // post event MainController.post(new
		 * UpdateUserPreferenceEvent(userPreference));
		 * 
		 * // hide window window.displayWindow(false, false);
		 */
	}
}
