		...
		if (cell != null) {
			if (cell instanceof mxCell) {
				if (((mxCell) cell).isVertex()) {
					...
					switch (label) {
					/*
					 * case UMS_UTILITY_RANKING: if (window.getController() !=
					 * null) { window.getUtilityView().setData(
					 * window.getController().getUserData(user));
					 * window.showUtilityFrame(true); } break;
					 */
					/*
					 * case UMS_SECURITY_AND_PRIVACY_FILTERING: if
					 * (window.getController() != null) {
					 * window.getPSView().setData(
					 * window.getController().getUserData(user));
					 * 
					 * window.showPSFrame(true); } break;
					 */
					default:
