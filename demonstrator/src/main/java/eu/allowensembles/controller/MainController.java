...
import eu.allowensembles.utils.ExternalEvent;
import eu.allowensembles.utils.HandlerInstance;
...
//	}
   ...
   /* public void processExternalEvents() {
	Iterator<ExternalEvent> iter = externalEvents.iterator();
	while (iter.hasNext()) {
	    ExternalEvent event = iter.next();
	    HandlerInstance handler = findHandler(event.getEvent());
	    if (handler != null) {
		TriggerEventType trigger = handler.getHandler()
			.getTriggerEvent();
		sendCollectiveAdaptationProblem(trigger.getName(),
			event.getEnsemble());
		iter.remove();
	    }
	}
